...
void update (int i, int S, int E, int index, int val)
{
	if (S == E && E == index)
	{
		tree[i].sum = tree[i].best = tree[i].bestLeft = tree[i].bestRight = a[index] = val;
		return;
	}
	int l, r, m;
	l = 2 * i;
	...
	m = S + (E-S) / 2;
	if (index <= m)
		update (l, S, m, index, val);
	else
		update (r, m+1, E, index, val);
